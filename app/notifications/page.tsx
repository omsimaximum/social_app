			...
			<Card>
				<CardHeader className='border-b'>
					<div className='flex items-center justify-between'>
						...
						<span className='text-muted-foreground text-sm'>{notifications.filter((n) => !n.read).length} unread</span>
					</div>
				</CardHeader>
					...
					<ScrollArea className='h-[calc(100vh-12rem)]'>
						{notifications.length === 0 ? (
							...
							notifications.map((notification) => (
								<div
									key={notification.id}
									className={`hover:bg-muted/25 flex items-start gap-4 border-b p-4 transition-colors ${!notification.read ? 'bg-muted/50' : ''}`}
								>
											...
											<div className='space-y-2 pl-6'>
												<div className='text-muted-foreground bg-muted/30 mt-2 rounded-md p-2 text-sm'>
													...
													{notification.post.image && (
														<img src={notification.post.image} alt='Post content' className='mt-2 h-auto w-full max-w-[200px] rounded-md object-cover' />
													)}
												</div>

												{notification.type === 'COMMENT' && notification.comment && (
													<div className='bg-accent/50 rounded-md p-2 text-sm'>{notification.comment.content}</div>
												)}
											</div>
										)}

										<p className='text-muted-foreground pl-6 text-sm'>{formatDistanceToNow(new Date(notification.createdAt), { addSuffix: true })}</p>
									</div>
								</div>
							))
						)}
